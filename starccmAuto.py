...
with open('batch.sh', 'w') as f:
    ...
    for i, sim in enumerate(simfulldir):
        f.write(starccmcommand + " -batch -power -np " + numofcores + " " + macrodir + sim + " -podkey lM66SSsU2JDB0BfHbD8gFA" +"\n")
        f.write("curl -u " + pushbullettoken + ": https://api.pushbullet.com/v2/pushes -d type=note -d title=\"" + simfiles[i] + " finished\"\n" )
